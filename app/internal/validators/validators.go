...
func NextDate(now time.Time, date string, repeat string) (string, error) {
	...
	// 	return "", fmt.Errorf("params `now` and `date must be in format YYYYMMDD\n")
	// }
...
func getDayOfMonth(date time.Time, space int) int {
	firstOfMonth := time.Date(date.Year(), date.Month(), 1, 0, 0, 0, 0, date.Location())
	LastOfMonth := firstOfMonth.AddDate(0, 1, space).Day()
	return LastOfMonth
}
